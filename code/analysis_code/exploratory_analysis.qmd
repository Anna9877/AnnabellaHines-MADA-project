...
```{r}
combo <- right_join(life, risk, by=c("Country", "Year")) %>% filter(Year==2015)
ggplot(combo, aes(x=GDP, y=Life.expectancy, color=Status))+geom_point()
```
...
Next I wanted to look at some vaccination rates in comparison to other factors.
```{r}
##Hepatitis.B, Polio, Diptheria
ggplot(combo, aes(y=Life.expectancy, x=Hepatitis.B))+geom_point()+geom_smooth()
ggplot(combo, aes(y=Life.expectancy, x=Polio))+geom_point()+geom_smooth()
ggplot(combo, aes(y=Life.expectancy, x=Diphtheria))+geom_point()+geom_smooth()
```
...
```{r}
p1 <- life %>% filter(Year==2000) %>% ggplot(aes(x=Status, y=Life.expectancy, fill=Status)) + geom_boxplot()+ylab("Life Expectancy")+xlab("Development Status")+labs(title="Life Expectancy Distribution by Development Status")
...
```{r}
fitlm5<-lm_mod %>% fit(Infant.deaths~Hepatitis.B+Polio+Diphtheria+Low.birth.weight.for.gestation, data=combo)
fitlm5
tidy(fitlm5)
```
